 ...
 * @param webhookUrl App 的 webhook URL（如 http://localhost:9902/hub/webhook）
 */
  ...
  const server = http.createServer(async (req, res) => {
    ...
    if (req.method === "POST" && url.pathname === "/mock/event") {
      try {
        ...
        // 构造 HubEvent（Slack 的 WxToSlack 从 data.from_id / from_name 读取发送者信息）
        const hubEvent = {
          v: 1,
          type: "event",
        ...
        // 计算签名（Slack 版 webhook 使用 x-timestamp 和 x-signature，签名为纯 hex）
