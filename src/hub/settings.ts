/**
 * /settings 页面 — 允许用户通过 app_token 验证身份后查看和修改 Slack 配置
 */
...
<head>
  ...
  <title>Slack Bridge — 设置</title>
  <style>${COMMON_STYLE}</style>
</head>
<body>
  <div class="card">
    <h1>Slack Bridge 设置</h1>
...
/**
 * POST /settings/save — 保存修改后的 Slack 配置
 */
export async function handleSettingsSave(
  ...
  if (slackBotToken) newConfig.slack_bot_token = slackBotToken;
  if (slackAppToken) newConfig.slack_app_token = slackAppToken;
  // slack_channel_id 允许清空，所以特殊处理：如果表单中有该字段则始终更新
  newConfig.slack_channel_id = slackChannelId;
...
function renderConfigForm(
  ...
  const maskedBotToken = maskValue(cfg.slack_bot_token || "");
  const maskedAppToken = maskValue(cfg.slack_app_token || "");
  const maskedChannelId = maskValue(cfg.slack_channel_id || "");
...
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>Slack Bridge — 修改配置</title>
  <style>${COMMON_STYLE}</style>
</head>
<body>
  <div class="card">
    <h1>Slack Bridge 设置</h1>
    <p class="desc">修改您的 Slack 应用凭证配置</p>
    ...
    <form method="POST" action="/settings/save">
      ...
      <input name="slack_bot_token" placeholder="留空则不修改" />
      ...
      <input name="slack_app_token" type="password" placeholder="留空则不修改" />
      <p class="hint">Settings → Socket Mode → App-Level Token</p>

      <label>Slack 频道 ID</label>
      <input name="slack_channel_id" placeholder="留空则清除" value="${escapeHtml(cfg.slack_channel_id || "")}" />
